"""
FIXED test_PPO.py
Corrected to use ImprovedA3CNet to match PPO training
"""
...
from model import ImprovedA3CNet  # PPO uses ImprovedA3CNet
...
def test_single_image(model, env, idx, save_path='test_result_ppo.png'):
    """Test PPO model on a single image"""
    
    model.eval()
    
    print(f"\n{'='*60}")
    print(f"Testing PPO on image index: {idx}")
    ...
    # Run agent - FIXED: Use 256 for ImprovedA3CNet
    state = env._state()
    h = torch.zeros(1, 256, 32, 32)  # 256 for ImprovedA3CNet
    ...
    axes[1,0].set_title(f"Registered Image (PPO)\nFinal MSE: {final_mse:.6f}\nImprovement: {improvement:+.1f}%", 
                       fontweight='bold', fontsize=13,
...
def test_all_images(model, env, results_dir):
    ...
    print(f"\n{'='*60}")
    print(f"Testing PPO model on {num_tests} test images...")
    print(f"{'='*60}\n")
    ...
    print(f"\n{'='*60}")
    print(f"PPO MODEL - TEST SET RESULTS")
    print(f"{'='*60}")
    print(f"\nMSE Improvement:")
    ...
    with open(results_dir / 'ppo_results.json', 'w') as f:
        json.dump(results_json, f, indent=2)
    
    print(f"\n✓ Results saved to: {results_dir / 'ppo_results.json'}")
    
    # Plot statistics
    fig = plt.figure(figsize=(15, 5))
    
    plt.subplot(1, 3, 1)
    plt.hist(improvements, bins=30, edgecolor='black', alpha=0.7, color='blue')
    plt.axvline(np.mean(improvements), color='darkblue', linestyle='--', linewidth=2,
                label=f'Mean: {np.mean(improvements):.1f}%')
    plt.xlabel('Improvement (%)', fontsize=12)
    plt.ylabel('Frequency', fontsize=12)
    plt.title('PPO: Distribution of Improvements', fontweight='bold', fontsize=14)
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plt.subplot(1, 3, 2)
    plt.scatter(initial_mses, final_mses, alpha=0.6, color='blue', s=20)
    ...
    plt.title('PPO: Initial vs Final MSE', fontweight='bold', fontsize=14)
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    bars = plt.bar(categories, rates, color='blue', alpha=0.7, edgecolor='black')
    for bar, rate in zip(bars, rates):
        height = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2., height,
                f'{rate:.1f}%', ha='center', va='bottom', fontweight='bold')
    plt.ylabel('Success Rate (%)', fontsize=12)
    plt.title('PPO: Success Rates', fontweight='bold', fontsize=14)
    plt.ylim(0, 100)
    plt.grid(True, alpha=0.3, axis='y')
    
    plt.tight_layout()
    plt.savefig(results_dir / 'ppo_statistics.png', dpi=150, bbox_inches='tight')
    print(f"✓ Statistics plot saved to: {results_dir / 'ppo_statistics.png'}")
    plt.show()
...
if __name__ == "__main__":
    ...
    MODEL_PATH = Path(r"C:\Users\Admin\Desktop\RL Image Registration\experiments\models\registration_ppo_safe_best.pth")
    RESULTS_DIR = Path(r"C:\Users\Admin\Desktop\RL Image Registration\results\PPO_results")
    ...
    if not MODEL_PATH.exists():
        print(f"\n❌ ERROR: PPO model not found at {MODEL_PATH}")
        print(f"Please run train_PPO_v2.py first!")
        exit(1)
    
    # FIXED: Use ImprovedA3CNet for PPO
    print("Using ImprovedA3CNet architecture (256 channels)...")
    model = ImprovedA3CNet()
    
    if not load_model_safe(MODEL_PATH, model):
        print("\n❌ Failed to load model. Please check:")
        print("  1. Model file exists")
        print("  2. Model was trained with ImprovedA3CNet")
    ...
    if choice == "1":
        ...
        test_single_image(model, env, idx, RESULTS_DIR / 'single_test_ppo.png')
    
    else:
        test_all_images(model, env, RESULTS_DIR)
    
    print("\n" + "="*60)
    print("PPO Testing Complete!")
    print("="*60)
    print(f"\nResults saved in: {RESULTS_DIR}")
    print("\nNext step: Run compare_algorithms.py to compare with A3C!")
    print("="*60 + "\n")